...
static llvm::cl::opt<bool>
    ltoSaveTemps("flto-save-temps", llvm::cl::desc("Save temporary LTO files"),
                 llvm::cl::Hidden);
...
std::string getLTOGoldPluginPath() {
  if (!ltoLibrary.empty()) {
    ...
    error(Loc(), "-lto-binary: file '%s' not found", ltoLibrary.c_str());
    fatal();
  } else {
    ...
    error(Loc(), "The LLVMgold.so plugin (needed for LTO) was not found. You "
                 "can specify its path with -lto-binary=<file>.");
    fatal();
  }
}
...
void addLTOGoldPluginFlags(std::vector<std::string> &args) {
  ...
  if (ltoSaveTemps)
    addLinkerFlag(args, "-plugin-opt=save-temps");
...
std::string getLTOdylibPath() {
  if (!ltoLibrary.empty()) {
    ...
    error(Loc(), "-lto-binary: '%s' not found", ltoLibrary.c_str());
    fatal();
  } else {
